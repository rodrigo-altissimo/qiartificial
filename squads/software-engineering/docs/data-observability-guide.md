...
> Agent: eng-data | Squad: software-engineering

## Overview
Freshness, volume, schema, distribution, lineage — 5 pillars.

## When to Use
- When the team needs guidance on data observability guide
...
## Related
- Agent: eng-data | Task: see tasks/ | Checklist: see checklists/


## Core Principles

### 1. Data Lifecycle (Reis & Housley)
Every data problem maps to: Generation → Storage → Ingestion → Transformation → Serving

```
GENERATION:   Where data originates (app DBs, APIs, IoT, logs)
STORAGE:      How it's persisted (lake, warehouse, lakehouse)
INGESTION:    Moving data (batch or streaming, CDC or full load)
TRANSFORMATION: Cleaning, enriching (dbt, Spark, SQL)
SERVING:      Delivering to consumers (dashboards, ML, APIs)

UNDERCURRENTS (at every stage):
  Security | Data Management | DataOps | Architecture | Orchestration | Engineering
```

### 2. Quality Is Non-Negotiable
Bad data is worse than no data. Quality checks at every stage.
...
### 3. Schema Is Contract
Schema changes are breaking changes. Evolve carefully.

```
SAFE:    Add optional column, add table, widen type
UNSAFE:  Remove column, rename column, change type, add NOT NULL

MIGRATION: Add → Backfill → Switch → Remove (weeks, not hours)
```
...
STEP 1 — REQUIREMENTS:
  ...
  Volume: [rows/day estimate]
  Quality requirements: [accuracy, completeness targets]

STEP 2 — ARCHITECTURE:
  Ingestion method:
    □ Full load (small tables < 1M rows)
    □ Incremental (updated_at > last_run)
    □ CDC (Debezium for real-time changes)
  
  Processing:
    □ Batch (Airflow + dbt): for daily/hourly
    □ Streaming (Kafka + Flink): for real-time
    □ Hybrid: streaming for operational, batch for analytical

  Storage layers:
    RAW:     Exact copy of source (no transformations)
    STAGING: Cleaned, typed, deduplicated (1:1 with source)
    INTERMEDIATE: Business logic applied (joins, calculations)
    MARTS:   Consumer-optimized (denormalized, aggregated)

STEP 3 — QUALITY GATES:
  After each layer, run quality checks:
    RAW → STAGING:
      □ Schema validation (expected columns, types)
      □ Row count comparison with source
    ...
    INTERMEDIATE → MARTS:
      □ Reconciliation with source (totals match)
      □ Distribution analysis (no anomalies)
      □ Freshness verification (updated within SLO)
```

### Data Quality Framework SOP
```
SIX DIMENSIONS OF DATA QUALITY:

1. ACCURACY: Does data match reality?
   Test: Compare sample against source of truth
   Threshold: > 99.9% accurate
   Tool: reconciliation queries

2. COMPLETENESS: Are all required fields present?
   Test: SELECT COUNT(*) WHERE field IS NULL
   Threshold: NULL rate < 1% on required fields
   Tool: dbt test not_null

3. CONSISTENCY: Does data agree across systems?
   Test: Compare counts/sums between source and destination
   Threshold: < 0.1% discrepancy
   Tool: cross-database reconciliation query

4. TIMELINESS: Is data fresh enough?
   Test: MAX(updated_at) < SLO threshold
   Threshold: per dataset (5 min for real-time, 24h for daily)
   Tool: dbt source freshness

5. UNIQUENESS: No unwanted duplicates?
   Test: COUNT(*) = COUNT(DISTINCT primary_key)
   Threshold: zero duplicates on PKs
   Tool: dbt test unique

6. VALIDITY: Does data conform to expected format?
   Test: Regex/enum validation on constrained fields
   Threshold: 100% valid on constrained fields
   Tool: dbt test accepted_values
```

### Schema Evolution SOP
```
SAFE CHANGES (no consumer notification needed):
  ✅ Add new nullable column
  ✅ Add new table
  ✅ Widen column type (INT → BIGINT)
  ✅ Add new index

UNSAFE CHANGES (30-day notification + migration plan):
  ❌ Remove column → expand-migrate-contract
  ❌ Rename column → add new, dual-write, migrate, drop old
  ❌ Change type (narrowing) → add new column, migrate
  ❌ Add NOT NULL → add with DEFAULT, backfill, then set NOT NULL

EXPAND-MIGRATE-CONTRACT PROTOCOL:
  Week 1: EXPAND — Add new column, dual-write
  Week 2: MIGRATE — Backfill historical data
  Week 3: VERIFY — All consumers using new column
  Week 4: CONTRACT — Drop old column (only after verification)
  
CONSUMER NOTIFICATION TEMPLATE:
  Subject: Schema Change Notice — [table_name]
  
  Change: [description]
  Type: [breaking/non-breaking]
  Timeline: [migration dates]
  Action required: [what consumers need to do]
  Support: [contact for questions]
```

### Data Observability SOP (5 Pillars)
```
FRESHNESS MONITORING:
  Query: SELECT MAX(updated_at) FROM dataset
  Alert: IF stale > SLO threshold
  Dashboard: Freshness gauge per dataset
  Owner: Data producer team

VOLUME MONITORING:
  Query: SELECT COUNT(*) per batch/run
  Alert: IF deviation > 30% from 7-day rolling average
  P0 Alert: IF zero rows loaded
  Dashboard: Volume trend chart per dataset

SCHEMA MONITORING:
  Track: Column names, types, constraints
  Alert: IF any column added/removed/type-changed
  Dashboard: Schema change log
  Integration: Schema registry (Confluent, AWS Glue)

DISTRIBUTION MONITORING:
  Track: min, max, mean, stddev, percentiles per key column
  Alert: IF statistical anomaly detected
  Example: Average order value suddenly 10x normal
  Dashboard: Distribution charts with anomaly highlighting

LINEAGE TRACKING:
  Track: Source → transformations → serving → consumers
  Tool: dbt lineage graph, DataHub, OpenLineage
  Use: Impact analysis for changes
  Dashboard: Interactive lineage graph
```


## Blueprint: Data Engineering Production Playbook

### 1. Data Pipeline Architecture Template
...
pipeline:
  ...
  slo:
    ...
    accuracy: "reconciliation within 0.01%"

source:
  system: order-api-postgres
  database: orders_production
  tables: [orders, order_items, customers]
  extraction_method: CDC (Debezium)
  change_data_capture:
    connector: debezium-postgres
    slot: orders_cdc_slot
    publication: orders_publication
    tables_included: [public.orders, public.order_items]
...
transformation:
  ...
  layers:
    raw:
      description: "Exact copy of source, append-only"
      materialization: incremental
      unique_key: _cdc_key
      
    staging:
      description: "Cleaned, typed, deduplicated"
      materialization: incremental
      tests:
    ...
    intermediate:
      description: "Business logic, joins, calculations"  
      models:
        - int_orders_with_items: "Orders joined with aggregated items"
        - int_customer_lifetime: "Customer LTV calculation"
      
    marts:
      description: "Consumer-optimized, denormalized"
      models:
        - mart_daily_revenue: "Revenue aggregated by day, category"
        - mart_customer_segments: "Customer segmentation by behavior"
...
serving:
  targets:
    - name: analytics-dashboard
      tool: Metabase
      tables: [mart_daily_revenue, mart_customer_segments]
      refresh: "every 15 minutes"
    
    - name: ml-features
      tool: Feature Store
      tables: [int_customer_lifetime]
      refresh: "hourly"

monitoring:
  freshness:
    check: "SELECT MAX(updated_at) FROM mart_daily_revenue"
    alert: "IF stale > 20 minutes"
    severity: P1
  
  volume:
    check: "SELECT COUNT(*) FROM staging WHERE batch_date = TODAY"
    alert: "IF deviation > 30% from 7-day average"
    severity: P2
    zero_rows_severity: P0
  
  reconciliation:
    check: "source SUM(total) vs warehouse SUM(total)"
  ...
  schema_changes:
    ...
    alert: "IF any column added/removed/changed"
    severity: P2
```

### 2. Data Quality Test Suite Template (dbt)

```yaml
# schema.yml — Complete test suite for orders model
version: 2

models:
  - name: stg_orders
    description: "Staged orders with cleaning and typing applied"
    columns:
      - name: order_id
