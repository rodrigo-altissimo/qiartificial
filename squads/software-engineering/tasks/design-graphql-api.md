---
task: design-graphql-api
squad: software-engineering
agent: eng-api
tags: [graphql, api, schema-design]
---

# Task: Design a GraphQL API

## Purpose
Design a GraphQL API with proper schema, query complexity limits, DataLoader patterns, and documentation.

## Steps

### 1. Identify Consumer Needs
```
Primary consumers: [web / mobile / internal service]
Why GraphQL over REST: [multiple consumers with different data needs / complex nested data / bandwidth optimization]
Data requirements per consumer:
  Consumer A needs: [fields]
  Consumer B needs: [fields]
```

### 2. Design Schema (Types)
```graphql
type Order {
  id: ID!
  customer: Customer!
...
### 3. Design Queries and Mutations
```graphql
type Query {
  order(id: ID!): Order
...
Identified N+1 risks:
  - order.customer → batch by customer_ids
  - order.items → batch by order_ids
DataLoader implementation:
  - Collect all IDs per resolver
  - Batch query: WHERE id IN (...)
  - Map results back
```

### 5. Set Query Complexity Limits
```
Max depth: 10 levels
...
Per-field costs:
  Scalar field: 1
  Object field: 5
  List field: 10 × estimated_size
Timeout: 10 seconds
Rate limit: 100 queries/min per client
```
...
## Quality Gate
- All N+1 queries prevented with DataLoader
- Query complexity limits enforced
- Schema reviewed by API consumers
