#!/usr/bin/env python3
"""
Brunson Squad — Handoff Validator
...
MANDATORY_FIELDS = [
    ...
    ("funnel_data_status.completeness", "Status Funnel Data (X/5)"),
...
CONDITIONAL_FIELDS = [
    {
        "field": "funnel_architecture.funnel_type",
        "condition": lambda data: get_nested(data, "transfer.current_phase", "").upper()
            in ["PHASE-1", "PHASE-2", "PHASE-3", "PHASE-4", "PHASE-FINAL"],
        "description": "Tipo de funil definido (obrigatorio apos PHASE-0)",
    },
    {
        "field": "funnel_architecture.pages_count",
        "condition": lambda data: get_nested(data, "transfer.current_phase", "").upper()
            in ["PHASE-2", "PHASE-3", "PHASE-4", "PHASE-FINAL"],
        "description": "Quantidade de paginas (obrigatorio apos PHASE-1)",
    },
    {
        "field": "funnel_architecture.conversion_targets",
        "condition": lambda data: get_nested(data, "transfer.current_phase", "").upper()
            in ["PHASE-2", "PHASE-3", "PHASE-4", "PHASE-FINAL"],
        "description": "Metas de conversao (obrigatorio apos PHASE-1)",
    },
]
