...
> Referência: `squads/design/data/high-retention-reading-guide.md`
...
*Checklist v1.0.0 — Baseado em WCAG 2.1/2.2, Apple HIG, Material Design 3, e praticas de publishers.*
...
|----------|-------|--------|
| Tipografia | 15 | 25% |
| Cor e Contraste | 14 | 20% |
| Acessibilidade Cognitiva | 10 | 15% |
| Elementos de UI para Leitura | 10 | 10% |
| Responsividade | 13 | 10% |
| Preferencias do Sistema | 7 | 10% |
| Suporte a Dislexia | 8 | 5% |
| Suporte a Baixa Visao | 6 | 5% |

**Minimum passing score:** 75%
**Blocking items (MUST pass regardless of overall score):**
- Body text >=16px (WCAG minimum)
- Texto normal contraste >=4.5:1 (AA)
- Texto alinhado a esquerda (NUNCA justificado)
- Zoom 200% funcional
...
| Failed Item | Fix |
|-------------|-----|
| Body text below 16px | Set `font-size: 1rem` (16px) minimum on body; use `clamp(1rem, 2.5vw, 1.25rem)` for fluid scaling |
| Text contrast below 4.5:1 | Adjust text color toward #212121 (light mode) or #E0E0E0 (dark mode); avoid pure black/white |
| Justified text found | Replace `text-align: justify` with `text-align: left`; search codebase with `grep -rn "text-align.*justify"` |
| Zoom 200% breaks layout | Replace fixed-width containers with `max-width: 80ch`; use relative units (rem, em) instead of px |
| prefers-reduced-motion not respected | Wrap animations in `@media (prefers-reduced-motion: no-preference)` query; provide static fallback |
