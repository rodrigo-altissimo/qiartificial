...
> Referencia: `squads/design/data/design-tokens-spec.yaml`
...
|----------|-------|--------|
| Cores (Primarias, Background, Texto, Semanticas) | 16 | 25% |
| Tipografia (Family, Size, Weight, Line Height) | 14 | 20% |
| Espacamento (Padding, Gap) | 10 | 15% |
| Bordas e Radius | 9 | 15% |
| Sombras | 5 | 5% |
| Motion | 7 | 10% |
...
- NUNCA hardcoded `#D4AF37` or `#B8962E` (primary color tokens must be used)
- Texto normal contraste >=4.5:1
- Zero hardcoded colors in validation command output

### Remediation Guide

| Failed Item | Fix |
|-------------|-----|
| Hardcoded color values found | Replace hex/rgb literals with design token references (`studio-primary`, `bg-background`, etc.) from `design-tokens-spec.yaml` |
| Hardcoded spacing (px values) | Replace arbitrary `p-[Xpx]` with standard Tailwind spacing tokens (`p-4`, `gap-6`, etc.) |
| Wrong border radius for component type | Consult radius mapping table above (Badge=sm, Button=md, Card=lg-xl, Modal=2xl, Avatar=full) |
| Contrast violation on dark background | Increase text opacity or switch to lighter text token; verify with WebAIM contrast checker |
| Missing transition on hover state | Add `transition-colors duration-200` to interactive elements; use `ease-out` for enter, `ease-in` for exit |
